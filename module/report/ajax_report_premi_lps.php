...
// if (floatval($tahun) <= '2016') {
// $val_harcode=9596797516;
// echo "kurang...";
...
WHERE a.JenisMataUang<>'IDR'and a.status_oncall='N'  and a.status_oncall='n'
)AS Valas 
GROUP BY FLAG
)AS DPK
GROUP BY FLAG
...
FROM $table_banklain a WITH (NOLOCK)
WHERE a.JenisMataUang<>'IDR'
AND a.FlagDPK='Deposito' and a.status_oncall='N'  and a.status_oncall='n'
)AS Valas
GROUP BY FLAG
)AS Bank_Lain
GROUP BY FLAG
...
$objPHPExcel->getActiveSheet()->setCellValue("F23", floatval($val_harcode));
} else {
