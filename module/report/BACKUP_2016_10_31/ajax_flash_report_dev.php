...
SELECT SUM(Nilai) AS Jumlah_Nominal FROM
(SELECT a.kodegl,a.kodeproduct,a.kodecabang,sum(a.nominal) AS Nilai FROM DM_Journal a WITH (NOLOCK)
left JOIN Referensi_GL_02 b ON b.GLNO = a.KodeGL AND b.PRODNO = a.KodeProduct
left JOIN Referensi_Flash_Report c ON c.FLASH_Level_3 = b.FLASH_LEVEL_3
WHERE a.DataDate='2016-05-26' AND 
b.FLASH_LEVEL_3 ='FLASH101000007' 
GROUP BY a.kodegl,a.kodeproduct,a.kodecabang,a.jenismatauang
)AS tabel1
...
//--------
/*
$q_perform_loan=" SELECT SUM(Nilai) AS Jumlah_Nominal FROM(
...
JOIN Referensi_Flash_Report c ON c.FLASH_Level_3 = b.FLASH_LEVEL_3
JOIN DM_AsetKredit d ON d.Managed_GL_Code = a.KodeGL AND d.DataDate = a.DataDate and d.managed_gl_prod_code=a.KodeProduct
WHERE a.DataDate='$curr_tgl' AND b.FLASH_Level_3 ='FLASH101000007' 
AND d.kolektibilitas IN ('1','2')
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
)AS tabel1 ";


        $result_perform_loan=odbc_exec($connection2, $q_perform_loan);
        $row_perform=odbc_fetch_array($result_perform_loan);
        $performing_loan=$row_perform['Jumlah_Nominal'];
//--------------
#perform currtgl_min1
...
JOIN Referensi_Flash_Report c ON c.FLASH_Level_3 = b.FLASH_LEVEL_3
JOIN DM_AsetKredit d ON d.Managed_GL_Code = a.KodeGL AND d.DataDate = a.DataDate and d.managed_gl_prod_code=a.KodeProduct
WHERE a.DataDate='$curr_tgl_min1' AND b.FLASH_Level_3 ='FLASH101000007' 
AND d.kolektibilitas IN ('1','2')
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
)AS tabel1 ";
        ...
        $result_perform_loan=odbc_exec($connection2, $q_perform_loan);
        $row_perform=odbc_fetch_array($result_perform_loan);
        $performing_loan_min1=$row_perform['Jumlah_Nominal'];
...
JOIN Referensi_Flash_Report c ON c.FLASH_Level_3 = b.FLASH_LEVEL_3
JOIN DM_AsetKredit d ON d.Managed_GL_Code = a.KodeGL AND d.DataDate = a.DataDate and d.managed_gl_prod_code=a.KodeProduct
WHERE a.DataDate='$curr_mon_min1' AND b.FLASH_Level_3 ='FLASH101000007' 
AND d.kolektibilitas IN ('1','2')
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
)AS tabel1 ";
        ...
        $result_perform_loan=odbc_exec($connection2, $q_perform_loan);
        $row_perform=odbc_fetch_array($result_perform_loan);
        $performing_loan_mon_min1=$row_perform['Jumlah_Nominal'];

#non
$q_non_perform_loan=" SELECT SUM(Nilai) AS Jumlah_Nominal FROM(
SELECT a.kodegl,SUM(a.nominal) AS Nilai FROM DM_Journal a WITH (NOLOCK)
JOIN Referensi_GL_02 b ON b.GLNO = a.KodeGL AND b.PRODNO = a.KodeProduct
JOIN Referensi_Flash_Report c ON c.FLASH_Level_3 = b.FLASH_LEVEL_3
JOIN DM_AsetKredit d ON d.Managed_GL_Code = a.KodeGL AND d.DataDate = a.DataDate 
and d.managed_gl_prod_code=a.KodeProduct 
WHERE a.DataDate='$curr_tgl' AND b.FLASH_Level_3 ='FLASH101000007'
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
)AS tabel1 ";


        $result_non_perform_loan=odbc_exec($connection2, $q_non_perform_loan);
        $row_non_perform=odbc_fetch_array($result_non_perform_loan);
        $non_performing_loan=$row_non_perform['Jumlah_Nominal'];
...
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
        ...
        $result_non_perform_loan=odbc_exec($connection2, $q_non_perform_loan);
        $row_non_perform=odbc_fetch_array($result_non_perform_loan);
        $non_performing_loan_min1=$row_non_perform['Jumlah_Nominal'];
...
GROUP BY a.kodegl ,b.FLASH_LEVEL_3 
        ...
        $result_non_perform_loan=odbc_exec($connection2, $q_non_perform_loan);
        $row_non_perform=odbc_fetch_array($result_non_perform_loan);
        $non_performing_loan_mon_min1=$row_non_perform['Jumlah_Nominal'];
...
<a href="<?php echo "http://".$_SERVER['HTTP_HOST']."/fincon_dev/module/report/"."download/Flash_Report_".$label_tgl."_".$file_eksport.".xls";?>" class="btn btn-sm green"> Download Excel  <i class="fa fa-arrow-circle-o-down"></i> </a> </div><br><br>
                    ...
                    <div class="portlet box grey-cascade" id="flash-report" >
                        <div class="portlet-title">
                            <div class="caption">
                                <i class="fa fa-book"></i> Flash Report
                            </div>
                            <div class="tools">
...
<tr class="warning">
                                                ...
                                                <td width="10%"><?=nice_number($profit_los)?></td>
                                                <td width="10%"><?=nice_number($profit_los3)?></td>
...
<?php
$dah_equity=$paid_in_capital+$agio_disagio+$general_reserve+$available_fss_net+$retained_earning+$profit_los;
$dah_equity3=$paid_in_capital3+$agio_disagio3+$general_reserve3+$available_fss_net3+$retained_earning3+$profit_los3;
